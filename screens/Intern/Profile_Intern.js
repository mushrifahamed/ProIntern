...
const Profile_Intern = () => {
  const [userData, setUserData] = useState({ fullName: '', email: '', mobileNumber: '' });
      ...
      if (currentUser) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            setProfilePic(userData.profilePicture || 'https://default-avatar-url.com/default-avatar.jpg');
            if (userData.cvUrl) {
  ...
  const handleProfilePicUpload = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const profilePicRef = ref(storage, `interns/${currentUser.uid}/${fileName}`);
        ...
        await updateDoc(doc(db, 'Interns', currentUser.uid), { profilePicture: downloadURL });
